...
//		ajax_sync = 1;
		$.ajax({
            ...
            contentType:ajax_params.contentType,
...
function checkInfo(divId){
	var datatype = {
			...
			"postNum":/^[0-9]{6}$/,
			//手机号码长度
			"phoneNum":/^13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|18[0-9]{9}$|17[0-9]{9}$/,
			//手机和电话号码长度
    ...
    var errorMsg = {
		   ...
		   "money":"金额只能输入数字！",
		   "ipAddr":"请输入正确的Ip地址，如192.168.1.1",
...
function onShow(that,ErrorMsg,type){
    ...
    that.css({"border-color":"#e56000","border-width":"1px"});
        if(type =="select2"){
